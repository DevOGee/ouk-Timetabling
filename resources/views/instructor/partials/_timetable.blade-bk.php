            ...
            <div style="width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; -ms-overflow-style: -ms-autohiding-scrollbar; background: #f8f9fa; padding: 5px 0;">
                <div style="min-width: 700px;">
                    ...
                    <div style="display: flex; margin-bottom: 5px;">
                        ...
                        @foreach($days as $day)
                            <div style="width: 140px; text-align: center; font-size: 0.75rem; font-weight: bold; padding: 3px; flex-shrink: 0;">
                                {{ $day->name }}
                            </div>
                        @endforeach
                    </div>
                    ...
                    @foreach (range($startTime, 20.5 * 60, 30) as $minute)
                        ...
                        @if ($timeLabel != '')
                            <div style="display: flex; min-height: 50px; border-top: 1px solid #dee2e6;">
                                <div style="width: 65px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; background: #f8f9fa; padding: 5px; position: sticky; left: 0; z-index: 2;">
                                    {{ $timeLabel }}
                                </div>
                                @foreach($days as $day)
                                    @php
                                    ...
                                    @if ($lesson)
                                        ...
                                        @endphp
                                        <div style="width: 140px; height: {{ $height }}px; margin: 0; background-color: {{ $color }}; color: white; padding: 5px; font-size: 0.7rem; border-radius: 3px; display: flex; flex-direction: column; justify-content: center; flex-shrink: 0;">
                                    ...
                                    @else
                                        <div style="width: 140px; height: 50px; margin: 0; background-color: #f8f9fa; border: 1px solid #dee2e6; flex-shrink: 0;"></div>
                                    @endif
                                @endforeach
                            </div>
                        @endif
    ...
    <div class="mt-5">
        <div class="card shadow-sm">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-sm table-bordered mb-0">
                        ...
                        <tbody>
                            @php
                                $sortedTimetable = $timetable->sortBy('day_id');
                            @endphp
                            @foreach($sortedTimetable as $index => $slot)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $days->firstWhere('id', $slot->day_id)->name ?? 'N/A' }}</td>
                                    <td>{{ $slot->courseUnit->code ?? 'N/A' }}</td>
                                    <td>{{ $slot->courseUnit->name ?? 'N/A' }}</td>
                                    <td>{{ $slot->programme->programme_code ?? 'N/A' }}</td>
                                    <td>
                                        @if($slot->morning_start_time)
                                            {{ \Carbon\Carbon::parse($slot->morning_start_time)->format('H:i') }} - 
                                            {{ \Carbon\Carbon::parse($slot->morning_start_time)->addMinutes($slot->morning_duration ?? 0)->format('H:i') }}
                                            ({{ $slot->morning_duration ?? 0 }} mins)
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        @if($slot->evening_start_time)
                                            {{ \Carbon\Carbon::parse($slot->evening_start_time)->format('H:i') }} - 
                                            {{ \Carbon\Carbon::parse($slot->evening_start_time)->addMinutes($slot->evening_duration ?? 0)->format('H:i') }}
                                            ({{ $slot->evening_duration ?? 0 }} mins)
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>{{ $slot->venue ?? 'TBA' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="card-footer small text-muted">
                Total {{ $timetable->count() }} slots | Generated: {{ now()->format('Y-m-d H:i:s') }}
            </div>
        </div>
    </div> --}}
@endif