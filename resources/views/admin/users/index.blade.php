        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <div class="dropdown">
                <button class="btn btn-outline-primary dropdown-toggle" type="button" data-bs-toggle="dropdown">
                    <i class="bi bi-camera-video me-1"></i> Zoom Licenses
                </button>
                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                    <li><h6 class="dropdown-header">Bulk Operations</h6></li>
                    <li>
                        <form method="POST" action="{{ route('admin.zoom.assign-thursday') }}"
                              onsubmit="return confirm('Assign Zoom licenses to all instructors with Thursday assignments (zoom_email not yet set)?')">
                            @csrf
                            <button type="submit" class="dropdown-item">
                                <i class="bi bi-check-circle text-success me-2"></i>
                                Assign — Thursday Instructors
                            </button>
                        </form>
                    </li>
                    <li>
                        <form method="POST" action="{{ route('admin.zoom.unassign-non-thursday') }}"
                              onsubmit="return confirm('Remove Zoom licenses from all instructors with NO Thursday assignments?')">
                            @csrf
                            <button type="submit" class="dropdown-item text-danger">
                                <i class="bi bi-x-circle me-2"></i>
                                Unassign — Non-Thursday Instructors
                            </button>
                        </form>
                    </li>
                    <li><hr class="dropdown-divider"></li>
                    <li>
                        <a class="dropdown-item text-muted small" href="{{ route('admin.settings.email') }}">
                            <i class="bi bi-gear me-2"></i>Email / SMTP Settings
                        </a>
                    </li>
                </ul>
            </div>
                                    ...
                                    <td>
                                        @if($user->zoom_email)
                                            <span class="badge bg-primary bg-opacity-10 text-primary"
                                                  data-bs-toggle="tooltip" title="{{ $user->zoom_email }}">
                                                <i class="bi bi-camera-video-fill me-1"></i>Licensed
                                            </span>
                                        @else
